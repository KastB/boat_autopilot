...
RotaryEncoder::RotaryEncoder(unsigned long interval, int pinA, int pinB,
                             int clearing) {
  m_interval = interval;
  m_pinA = pinA;
  m_pinB = pinB;
  m_clearing = clearing;  // if measured voltage changes (to the right
                          // direction) by more than this the state is swapped
  m_currentPosition = 0;
  // position
  m_voltageA = analogRead(m_pinA);
  m_voltageB = analogRead(m_pinB);
  if (m_voltageA > 530) m_aHigh = true;
  if (m_voltageB > 530) m_bHigh = true;
}
...
void RotaryEncoder::update() {
  ...
  // a
  if (stateA) {
    if (a < m_voltageA - m_clearing) stateA = false;
  ...
  // b
  if (stateB) {
    if (b < m_voltageB - m_clearing) stateB = false;
...
int RotaryEncoder::getCurrentPosition() { return m_currentPosition; }

void RotaryEncoder::setCurrentPosition(int pos) { m_currentPosition = pos; }
